...
$pekan = 1;
$sql2 = "select distinct(pekan) from realisasi_lagmeasure1 where nipeg = '$user'";
...
if ($result2->num_rows > 0) {
    ...
    <td>'.$row["tanggal"].'</td>		
    <td><a href="Form_PLT443.php?session='.$session.'&pekan='.$pekan.'&user2='.$user.'">Detail</a></td>
    </tr>';
	$pekan = $pekan +1;
    }
}
...
echo '<a href="Form_PLT44.php?session='.$session.'" class="btn btn-primary btn-lg active spacer2 btn-sm" role="button">Isi Formulir</a>';
$conn->close();
?>
</div>
