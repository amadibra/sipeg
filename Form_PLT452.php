...
$sql2 = "SELECT * FROM laporan_pelaksanaan where nipeg = '$user2'";
$result2 = $conn->query($sql2); 
	  if ($result2->num_rows > 0) {
    // output data of each row
    while($row = $result2->fetch_assoc()) {
...
echo '<div class="col-sm-5">';
	...
	echo '<textarea class="form-control" rows="3" name="feedback_pelaksanaan"></textarea>';
	echo '</div>';
  echo '</div>';
...
if ($result->num_rows > 0) {
	...
	{
		echo '<div id="tombol">';
		echo '<button type="submit" class="btn btn-warning" name="tombol5" value="1" >Setujui</button>';
		echo '</div>';
		
		echo '<div id="tombol">';
		echo '<button type="submit" class="btn btn-success" name="tombol5" value="0" >Batal Setujui</button>';
		echo '</div>';
	}
	else{
		echo '<div id="tombol">';
