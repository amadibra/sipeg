...
$sql = "SELECT lagmeasure".$y." FROM lagmeasure".$y." where nipeg = '$user2'";
$result = $conn->query($sql);
...
echo '<div class="col-sm-5">';
  ...
  $sql = "SELECT feedback FROM wig where nipeg = '$user2'";
$result = $conn->query($sql);
...
$status = null;  
$sql = "SELECT status FROM wig where nipeg = '$user2'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
...
<input type="text" class="form-control" placeholder="Text input">
   ...
   <?php 
  echo "<a href='delete_wig.php?session=".$session."' class='btn btn-primary btn-lg active spacer2 btn-sm' role='button'>Buat Ulang</a>";
