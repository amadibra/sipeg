...
$sql = "SELECT lagmeasure".$y.",kuantitas,kualitas,waktu FROM lagmeasure".$y." where nipeg = '$user2'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    echo '<div class="col-sm-5">';
	echo '<p class="form-control-static">'.$row["kuantitas"].' </p>';
	echo '</div>';
	echo '</div>';
    ...
    echo '<div class="col-sm-5">';
	echo '<p class="form-control-static">'.$row["kualitas"].' </p>';
	echo '</div>';
	echo '</div>';
    ...
    echo '<div class="col-sm-5">';
	echo '<p class="form-control-static">'.$row["waktu"].' </p>';
	echo '</div>';
	echo '</div>';
...
echo '<div class="col-sm-5">';
	...
	echo '<textarea class="form-control" rows="3" name="feedback_timeline"></textarea>';
	echo '</div>';
  echo '</div>';
  
  $status = null;  
$sql = "SELECT status_timeline FROM wig where nipeg = '$user2'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
	if($row["status_timeline"] == 0)
	{
		echo '<div id="tombol">';
		echo '<button type="submit" class="btn btn-warning" name="tombol2" value="1" >Setujui</button>';
		echo '</div>';
		
		echo '<div id="tombol">';
		echo '<button type="submit" class="btn btn-success" name="tombol2" value="0" >Batal Setujui</button>';
		echo '</div>';
	}
	else{
		echo '<div id="tombol">';
...
<input type="text" class="form-control" placeholder="Text input">
    ...
    <?php
  echo "<td ><strong> <font size=2 ><a href='print42.php?session=".$session."&nipeg=$user&pos=$kd_posisi' target='_blank' ><img src=images/print_icon.png></a></font></strong></td>";
  ?>
</form>
</div>
